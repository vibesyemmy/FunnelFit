...
interface SignInPageProps {
  onNavigate: (page: 'sign-in' | 'sign-up' | 'create-account' | 'email-verification' | 'verification-success' | 'onboarding' | 'onboarding-success' | 'sme-dashboard' | 'sme-main-dashboard' | 'upload-center' | 'bank-connection' | 'cfo-dashboard', accountType?: 'sme' | 'cfo', email?: string) => void
}
...
const SignInPage: React.FC<SignInPageProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (validateForm()) {
      // Simulate sign in logic with predefined credentials
      console.log('Sign in:', formData)
      
      // Demo credentials for testing
      const credentials = {
        // SME credentials
        'sme@funnelfit.com': { password: 'sme123', accountType: 'sme' as const },
        'business@funnelfit.com': { password: 'business123', accountType: 'sme' as const },
        
        // CFO credentials  
        'cfo@funnelfit.com': { password: 'cfo123', accountType: 'cfo' as const },
        'advisor@funnelfit.com': { password: 'advisor123', accountType: 'cfo' as const }
      }
      
      const userCredentials = credentials[formData.email as keyof typeof credentials]
      ...
      if (userCredentials.accountType === 'cfo') {
        onNavigate('cfo-dashboard', userCredentials.accountType, formData.email)
      } else {
        onNavigate('sme-main-dashboard', userCredentials.accountType, formData.email)
      }
    }
  }

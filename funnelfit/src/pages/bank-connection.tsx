...
interface BankConnectionProps {
  onNavigate: (page: 'sign-in' | 'sign-up' | 'create-account' | 'email-verification' | 'verification-success' | 'onboarding' | 'onboarding-success' | 'sme-dashboard' | 'upload-center' | 'bank-connection', accountType?: 'sme' | 'cfo', email?: string) => void
  accountType: 'sme' | 'cfo'
  email: string
}
...
interface Bank {
  ...
  connectionMethod: 'api' | 'manual' | 'both'
}

const BankConnection: React.FC<BankConnectionProps> = ({ 
  onNavigate, 
  accountType, 
  email 
}) => {
  ...
  const nigerianBanks: Bank[] = [
    {
      ...
      logo: 'AB',
      hasApi: true,
      supportedAccounts: ['Savings', 'Current', 'Domiciliary'],
    ...
    },
    {
      ...
      logo: 'FD',
      hasApi: false,
      supportedAccounts: ['Savings', 'Current'],
  ...
  const handleConnectBank = () => {
    if (selectedBank) {
      ...
      // In a real app, this would initiate the bank connection process
      // For now, just navigate back to dashboard
      onNavigate('sme-dashboard', accountType, email)
    }
  }

  const getConnectionMethodInfo = (method: string) => {
    switch (method) {
      case 'api':
        return {
          label: 'Direct Connection',
          description: 'Connect securely via bank API',
      ...
      case 'manual':
        return {
          label: 'Manual Upload',
          description: 'Upload bank statements manually',
          color: 'text-blue-600',
          bgColor: 'bg-blue-50',
      ...
      case 'both':
        return {
          ...
          description: 'Choose API or manual upload',
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600">
            Connect your bank accounts to automatically import transactions and get real-time financial data.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="space-y-3">
            {filteredBanks.map((bank) => {
              const connectionInfo = getConnectionMethodInfo(bank.connectionMethod)
              return (
                <Card 
                  key={bank.id} 
                  className={`hover:shadow-md transition-shadow cursor-pointer ${
                    selectedBank?.id === bank.id ? 'ring-2 ring-primary-500' : ''
                  }`}
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center space-x-4">
                      <div className="w-10 h-10 bg-primary-100 rounded-full flex items-center justify-center flex-shrink-0">
                        <span className="text-sm font-semibold text-primary-700">{bank.logo}</span>
                      </div>
                      ...
                      <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${connectionInfo.bgColor} ${connectionInfo.color}`}>
                        {connectionInfo.icon}
                        <span className="ml-1">{connectionInfo.label}</span>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        {selectedBank && (
          <div className="mt-8 flex justify-center">
            ...
            >
              ...
              <span>Connect to {selectedBank.name}</span>
            </Button>
          </div>
        )}

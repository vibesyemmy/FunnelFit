...
import { Building2Icon, UserCheckIcon, ArrowLeftIcon } from 'lucide-react'
...
const SignUpPage: React.FC<SignUpPageProps> = ({ onNavigate }) => {
  const [selectedType, setSelectedType] = useState<'sme' | 'cfo' | null>(null)

  const handleAccountTypeSelect = (type: 'sme' | 'cfo') => {
  ...
  const handleContinue = () => {
    if (selectedType) {
      ...
      onNavigate('create-account', selectedType)
    }
  }
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <div className="space-y-4 mb-8">
            ...
            <CardContent className="p-6">
              <div className="flex items-start space-x-4">
                <div className={`p-3 rounded-lg ${
                  selectedType === 'sme' ? 'bg-primary-100' : 'bg-gray-100'
                }`}>
                  <Building2Icon className={`h-6 w-6 ${
                    selectedType === 'sme' ? 'text-primary-600' : 'text-gray-600'
                  }`} />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">
                    Small & Medium Business
                     ...
                     I'm a business owner looking for fractional CFO services to help grow my company.
                   </p>
                </div>
                {selectedType === 'sme' && (
          ...
          <Card 
            ...
            onClick={() => handleAccountTypeSelect('cfo')}
          >
            <CardContent className="p-6">
              <div className="flex items-start space-x-4">
                <div className={`p-3 rounded-lg ${
                  selectedType === 'cfo' ? 'bg-primary-100' : 'bg-gray-100'
                }`}>
                  <UserCheckIcon className={`h-6 w-6 ${
                    selectedType === 'cfo' ? 'text-primary-600' : 'text-gray-600'
                  }`} />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">
                    Fractional CFO
                                     ...
                                     <p className="text-sm text-gray-600">
                     I'm a financial professional looking to provide CFO services to growing businesses.
                   </p>
                </div>
                {selectedType === 'cfo' && (
                  <div className="w-5 h-5 bg-primary-500 rounded-full flex items-center justify-center">
